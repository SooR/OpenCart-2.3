...
class ControllerStartupSeoPro extends Controller {
	...
	public function index() {
		...
		if(isset($this->request->get['_route_'])){
			...
			if(isset($urllanguage[0]) && in_array($urllanguage[0], $city)){
				if(count($urllanguage) > 1){
					$replace_city = $urllanguage[0] . "/";
				}else{
					$replace_city = $urllanguage[0];
				}
				
				$this->request->get['_route_'] = str_replace($replace_city, '', $this->request->get['_route_']);
			}
		}
		// End City Mod
	...
	public function rewrite($link) {
		...
		// City Mod
		if($main_page || $category_page) {
			
			$config_city = trim($this->config->get('config_city'));
